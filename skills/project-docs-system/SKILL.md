...
3. Sort knowledge before writing.
   ...
   - Keep current knowledge in the living document; move decision history into an adjacent `<stem>.adr.md` with relative Markdown links in both directions. Create it only when there is history to preserve. See [the companion template](./references/adr-template.md).
   - Keep companions out of index entries and generated catalogs. Read them only when the linked living document makes the historical reasoning relevant.
...
5. Keep maps complete.
   ...
   - When adding, renaming, merging, or removing living Markdown docs files, update the nearest parent `index.md` in the same change.
...
- When migrating inline decision records, move their history into the companion without changing its meaning or chronology; retain current behavior in the living document and add both links. Use `[Decision history](./<stem>.adr.md)` and `[Current documentation](./<stem>.md)`.
- Preserve earlier rationale when decisions change. A planned decision does not establish implemented behavior.
- When verification warns that a document exceeds 500 author-maintained lines, review it for accumulated ADRs, mixed responsibilities, and unnecessary reading context. Extract history, split cohesive domains, or trim content as appropriate; do not split mechanically to satisfy the threshold. This applies to companions too; generated catalog blocks are excluded.
...
- Confirm each docs scope `index.md` covers its direct living Markdown docs files and child docs scope indexes; companion ADRs are excluded.
- Confirm each companion has a living document and relative Markdown links in both directions.
